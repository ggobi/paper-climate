...
locs2 <- data.frame(cbind(rep(t+pi,12), rep(mean(locs$temp), 12*36), c(rep(1:6, each=36), rep(1:6, each=36)), rep(2:1,each=36*6)))
...
ggplot(locs2, aes(gx, gy, group = gid)) + 
  geom_path(colour="white", size=1.5) +
...
ggplot(locs, aes(gx, gy, group = gid)) + 
  geom_path(aes(gx, gy, group = gid), data=locs2, colour="white", size=1.5) +
  geom_path() +
  geom_tile(aes(x_max, y_max), colour = "white", fill = NA) +
  theme_fullframe() + coord_equal()
