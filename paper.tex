...
Figure \ref{fig:gistemp-pred} (left) shows the time series icons and star glyphs drawn for each spatial location of the predicted temperature data across the USA, we call it a \emph{glyph-map}. The data contains 2$^o$x2$^o$ gridded temperature anomalies 1950-2010, from the GISTEMP surface temperature data provided by the Earth System Research Laboratory, Physical Sciences Division, National Oceanic and Atmospheric Administration. Most locations observe an increasing temperature, to varying degrees, over this time period. A few locations show a decline. These locations tend to be different from their neighbors, which might suggest isolated data problems.
...
\begin{figure}[htbp]
  ...
  \caption{Glyph-maps of gridded predicted temperature anomaly data}
