...
\begin{figure}[htbp]
  ...
  \caption{Examples for some small time series with strongly expressed trends in Euclidean coordinates (left) and polar coordinates (right).}
  \label{fig:ref-basic-polar}
\end{figure}
...
\begin{equation}
y_{\amaj} = \frac{y_{\amaj}-y_{min}}{y_{max}-y_{min}} \times 2 - 1
\end{equation}

\noindent where $y_{min} = \mbox{minimum}\{y_{st}; s=1, \dots S, t=1, ..., T \}, y_{max} = \mbox{maximum}\{y_{st}; s=1, \dots S, t=1, ..., T\}$, $S=$ is the number of spatial locations and $T=$ is the number of time points. *** Reference other plots that use global scale too.
...
\begin{equation}
y_{\amaj} = \frac{y_{\amaj}-y_{s,min}}{y_{s,max}-y_{s,min}} \times 2 - 1
\end{equation}
...
*** Connection to small multiples

*** Scale of icons relative to data or model range
